...
{
    public class Dialog_LoadTeleporter : Window
    {
		...
		public Dialog_LoadTeleporter(Map map, List<CompTransporter> transporters)
		{
			this.map = map;
		...
		public override void DoWindowContents(Rect inRect)
		{
			...
			Dialog_LoadTeleporter.tabsList.Clear();
			Dialog_LoadTeleporter.tabsList.Add(new TabRecord("PawnsTab".Translate(), delegate ()
			{
				this.tab = Dialog_LoadTeleporter.Tab.Pawns;
			}, this.tab == Dialog_LoadTeleporter.Tab.Pawns));
			Dialog_LoadTeleporter.tabsList.Add(new TabRecord("ItemsTab".Translate(), delegate ()
			{
				this.tab = Dialog_LoadTeleporter.Tab.Items;
			}, this.tab == Dialog_LoadTeleporter.Tab.Items));
			inRect.yMin += 67f;
			Widgets.DrawMenuSection(inRect);
			TabDrawer.DrawTabs(inRect, Dialog_LoadTeleporter.tabsList, 200f);
			...
			inRect2.yMax -= 59f;
			bool flag = false;
			Dialog_LoadTeleporter.Tab tab = this.tab;
			if (tab != Dialog_LoadTeleporter.Tab.Pawns)
			{
				if (tab == Dialog_LoadTeleporter.Tab.Items)
				{
		...
		private void DoBottomButtons(Rect rect)
		{
			...
			if (Widgets.ButtonText(rect2, "TeleportButton".Translate(), true, true, true))
			{
					...
					if (this.CheckForErrors(TransferableUtility.GetPawnsFromTransferables(this.transferables)))
					{
						Find.WindowStack.Add(Dialog_MessageBox.CreateConfirmation("TransportersCaravanWillBeImmobile".Translate(), delegate
						{
							if (this.DebugTryLoadInstantly())
							{
								...
								this.Close(false);
								this.transporters[0].parent.TryGetComp<Comps.ItemComps.CompArchotechTeleporter>().StartChoosingDestination();
							}
						}, false, null));
					}
				}
				else if (this.DebugTryLoadInstantly())
				{
					SoundDefOf.Tick_High.PlayOneShotOnCamera(null);
					this.Close(false);
					this.transporters[0].parent.TryGetComp<Comps.ItemComps.CompArchotechTeleporter>().StartChoosingDestination();
		...
		{
			Dialog_LoadTeleporter.tmpLeftToLoadCopy.Clear();
			for (int i3 = 0; i3 < this.transporters.Count; i3++)
			{
				Dialog_LoadTeleporter.tmpLeftToLoadCopy.Add((this.transporters[i3].leftToLoad != null) ? this.transporters[i3].leftToLoad.ToList<TransferableOneWay>() : new List<TransferableOneWay>());
				if (this.transporters[i3].leftToLoad != null)
				{
					this.transporters[i3].leftToLoad.Clear();
				}
			}
			Dialog_LoadTeleporter.tmpLeftCountToTransfer.Clear();
			for (int j = 0; j < this.transferables.Count; j++)
			{
				Dialog_LoadTeleporter.tmpLeftCountToTransfer.Add(this.transferables[j], this.transferables[j].CountToTransfer);
			}
			if (this.LoadingInProgressOrReadyToLaunch)
			{
				int i2;
				...
				{
					if (this.transferables[i].HasAnyThing && Dialog_LoadTeleporter.tmpLeftCountToTransfer[this.transferables[i]] > 0)
					{
						for (int k = 0; k < Dialog_LoadTeleporter.tmpLeftToLoadCopy.Count; k++)
						{
							TransferableOneWay transferableOneWay = TransferableUtility.TransferableMatching<TransferableOneWay>(this.transferables[i].AnyThing, Dialog_LoadTeleporter.tmpLeftToLoadCopy[k], TransferAsOneMode.PodsOrCaravanPacking);
							if (transferableOneWay != null)
							{
								int num = Mathf.Min(Dialog_LoadTeleporter.tmpLeftCountToTransfer[this.transferables[i]], transferableOneWay.CountToTransfer);
								if (num > 0)
								{
									this.transporters[k].AddToTheToLoadList(this.transferables[i], num);
									Dictionary<TransferableOneWay, int> dictionary = Dialog_LoadTeleporter.tmpLeftCountToTransfer;
									TransferableOneWay key = this.transferables[i];
									dictionary[key] -= num;
								}
							}
							IEnumerable<Thing> innerContainer = this.transporters[k].innerContainer;
							Thing thing = innerContainer.FirstOrDefault((Thing x) => TransferableUtility.TransferAsOne(this.transferables[i].AnyThing, x, TransferAsOneMode.PodsOrCaravanPacking));
							if (thing != null)
							{
								int num2 = Mathf.Min(Dialog_LoadTeleporter.tmpLeftCountToTransfer[this.transferables[i]], thing.stackCount);
								if (num2 > 0)
								{
			...
			{
				TransferableOneWay transferableOneWay2 = this.transferables.MaxBy((TransferableOneWay x) => Dialog_LoadTeleporter.tmpLeftCountToTransfer[x]);
				int num3 = 0;
				for (int l = 0; l < this.transferables.Count; l++)
				{
					if (this.transferables[l] != transferableOneWay2 && Dialog_LoadTeleporter.tmpLeftCountToTransfer[this.transferables[l]] > 0)
					{
						this.transporters[num3 % this.transporters.Count].AddToTheToLoadList(this.transferables[l], Dialog_LoadTeleporter.tmpLeftCountToTransfer[this.transferables[l]]);
						num3++;
					}
				}
				if (num3 < this.transporters.Count)
				{
					int num4 = Dialog_LoadTeleporter.tmpLeftCountToTransfer[transferableOneWay2];
				...
				{
					this.transporters[num3 % this.transporters.Count].AddToTheToLoadList(transferableOneWay2, Dialog_LoadTeleporter.tmpLeftCountToTransfer[transferableOneWay2]);
				}
			}
			Dialog_LoadTeleporter.tmpLeftCountToTransfer.Clear();
		...
		private Dialog_LoadTeleporter.Tab tab;
