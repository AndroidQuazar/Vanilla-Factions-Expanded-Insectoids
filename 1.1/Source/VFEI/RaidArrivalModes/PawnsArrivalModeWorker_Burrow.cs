...
{
    class PawnsArrivalModeWorker_Burrow : PawnsArrivalModeWorker
    {
		...
		public static void InsectTunnel(IncidentParms parms, List<Pawn> pawns)
		{
			...
			for (int i = 0; i < 15; i++)
			{
				...
				MoteMaker.MakeStaticMote(intvr2, map, ThingDefOf.Mote_DustPuffThick, Rand.Range(1.5f, 3f));

				if (point - 100 > 0 && Rand.Bool)
				{
					Thing thingR = ThingMaker.MakeThing(ThingDefsVFEI.VFEI_BurrowRoyal);
					thingR.SetFaction(faction);
					GenSpawn.Spawn(thingR, intvr, map);
					pawns.Add(PawnGenerator.GeneratePawn(ThingDefsVFEI.VFEI_Insectoid_Gigalocust, faction));
					point -= 100;
				}
				else if (point - 75 > 0 && Rand.Bool)
				{
					Thing thingM = ThingMaker.MakeThing(ThingDefsVFEI.VFEI_BurrowMedium);
				...
				{
					Thing thingS = ThingMaker.MakeThing(ThingDefsVFEI.VFEI_BurrowSmall);
